...
def get_task_instruction(instance: dict, task: str = 'auto_search', include_pr=False, include_hint=False):
    ...
    if task.strip() == 'auto_search':
        ...
        # )
        task_description = auto_search.TASK_INSTRUECTION
...
def auto_search_process(result_queue,
    ...
    if separate_api_keys == 'True':
        api_key = os.getenv('LOCAGENT_API_KEY')
        if not api_key:
            raise ValueError("Environment variable 'LOCAGENT_API_KEY' is not set. Please set this variable to use separate API keys.")
        try:
            client = OpenAI(
            ...
            logging.error(f"Failed to initialize OpenAI client with LOCAGENT_API_KEY: {e}")
            raise
    else:
        api_key = os.getenv('API_KEY')
        if not api_key:
            raise ValueError("Environment variable 'API_KEY' is not set. Please set this variable before running.")
        try:
            client = OpenAI(
                base_url="https://openrouter.ai/api/v1",
                api_key=api_key,
            ...
            logging.error(f"Failed to initialize OpenAI client with API_KEY: {e}")
            raise
            ...
            while max_attempt_num:
                ...
                try:
                    ...
                    process = ctx.Process(target=auto_search_process, kwargs={
                        ...
                        'separate_api_keys': args.separate_keys,
                    })
                    process.start()
...
def localize(args):
    # Directly assign the dataset for the custom issue
    custom_issue = {
        "instance_id": args.instance_id,
        "problem_statement": args.query,  # Custom query passed by user
        "repo": args.repo,                # Custom repo passed by user
        "base_commit": "latest",
        "patch": ""
    }

    # Use a single custom issue as the task, instead of loading a whole dataset
    bench_tests = [custom_issue]
    
    # Limit evaluation if required
    ...
    # Continue with the rest of the processing as usual
    manager = mp.Manager()
    queue = manager.Queue()
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["gpt-4o", 
                 "azure/gpt-4o", "openai/gpt-4o-2024-05-13", "openrouter/google/gemini-2.5-pro-exp-03-25:free", "openrouter/rekaai/reka-flash-3:free", "openrouter/cognitivecomputations/dolphin3.0-mistral-24b:free",
                 "deepseek/deepseek-chat", "deepseek-ai/DeepSeek-R1", "openrouter/qwen/qwq-32b:free", "openrouter/qwen/qwen2.5-vl-32b-instruct:free",
                 "openrouter/mistralai/mistral-small-24b-instruct-2501:free", "openrouter/deepseek/deepseek-chat:free",
                 "litellm_proxy/claude-3-5-sonnet-20241022", "openrouter/qwen/qwq-32b-preview:free", "openrouter/nvidia/llama-3.1-nemotron-70b-instruct:free", "litellm_proxy/gpt-4o-2024-05-13", "litellm_proxy/o3-mini-2025-01-31", "openrouter/qwen/qwen-2.5-coder-32b-instruct:free", "openrouter/qwen/qwen-2.5-72b-instruct:free",
                 "openrouter/google/gemini-2.0-flash-thinking-exp-1219:free", "openrouter/google/gemini-2.0-flash-exp:free",
                 "openrouter/google/gemini-exp-1206:free", "openrouter/meta-llama/llama-3.3-70b-instruct:free",
                 # fine-tuned model
    ...
    parser.add_argument("--query", required=True, help="Custom query for localization")
    parser.add_argument("--repo", required=True, help="Path to the local repository")
    parser.add_argument("--separate_keys", required=True, help="A boolean flag determining whether to separate api keys for each agent should be used")

    parser.add_argument("--instance_id", type=str, default="MySampleProblem54", help="Custom instance ID for localization")
