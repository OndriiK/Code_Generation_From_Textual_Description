...
import re
from LocAgent.util.benchmark.setup_repo import setup_repo
...
file_path_str = os.path.dirname(os.path.abspath(__file__))
...
try:
    # Find the index of 'LocAgent'
    locagent_index = parts.index("LocAgent")

    base_parts = parts[:locagent_index]

    # The Path constructor can take an unpacked sequence of path parts.
    base_path_for_new_folder = Path(*base_parts)

    # Concatenate with the new folder name
    graph_index_folder = base_path_for_new_folder / "loc_agent_outputs_resources/graph_index_dir"
    bm25_index_folder = base_path_for_new_folder / "loc_agent_outputs_resources/bm25_index_dir"
except ValueError:
    # Handle the case where 'LocAgent' is not in the path if necessary
    graph_index_folder = None


GRAPH_INDEX_DIR = graph_index_folder
BM25_INDEX_DIR = bm25_index_folder
