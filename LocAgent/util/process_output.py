...
from LocAgent.util.benchmark.parse_python_file import (
    parse_global_var_from_code, is_global_var
)
from LocAgent.dependency_graph import RepoEntitySearcher
from LocAgent.dependency_graph.build_graph import (
    NODE_TYPE_FILE, NODE_TYPE_FUNCTION, NODE_TYPE_CLASS
)
import pickle
import os
from LocAgent.plugins.location_tools.utils.util import GRAPH_INDEX_DIR
...
def parse_raw_loc_output(raw_output, valid_files):
    ...
    for line in lines:
        ...
        if line.endswith('.py'):
            fn = extract_python_file_path2(line, valid_top_folder)
...
def extract_python_file_path2(line, valid_files):
    """
    Extracts a Python file path from a line of text and checks if it's in the valid_files list.

    Parameters:
    - line (str): A line of text that may contain a Python file path.
    - valid_files (list): List of valid Python file paths.

    Returns:
    - str or None: The matched file path if it's in valid_files, else None.
    """
    pattern = r'(?:[\w\-/]+/)*[\w\-]+\.py'
    match = re.search(pattern, line)
    ...
    if match:
        matched_fp = match.group(0)
        # Check against valid files (exact match)
        if matched_fp in valid_files:
            return matched_fp
        # Check if the file is matched by filename only
        matched_filename = matched_fp.split('/')[-1]
        for vf in valid_files:
            if vf.endswith(matched_filename):
                return vf
    return None
