...
from LocAgent.util.benchmark.git_repo_manager import setup_github_repo
import argparse
...
def setup_repo(
    ...
    if instance_data["repo"]:
        repo_path = instance_data["repo"]

    # Check if it's a local path and use it directly
    if os.path.exists(repo_path):
        print(f"!!!!!!Using local repository at {repo_path}")
        return repo_path  # Use the local repository directly
