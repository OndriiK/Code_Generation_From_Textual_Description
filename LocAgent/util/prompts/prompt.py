...
class PromptManager:
    ...
    def __init__(
        ...
        self.myuser_template: Template = self._load_template('myuser_prompt')
    ...
    def load_and_render_user_prompt(self, template_name: str, context: dict = None) -> str:
        """
        Loads and renders a user prompt template given its name.

        Args:
            template_name (str): The name of the template (without '.j2').
            context (dict, optional): A dictionary of context variables for rendering.
    ...
    @property
    def myuser_message(self) -> str:
        rendered = self.myuser_template.render(
            micro_agent=None
        ).strip()
        return rendered
