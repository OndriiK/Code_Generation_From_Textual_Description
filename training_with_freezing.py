...
# Step 6: Freeze and Unfreeze Logic
logger.info("Freezing model layers...")
for name, param in model.named_parameters():
    if "transformer.h" in name:  # Freeze all transformer layers initially
...
class UnfreezeCallback(TrainerCallback):
    ...
    def on_epoch_begin(self, args, state, control, **kwargs):
        if state.epoch >= self.unfreeze_after_epoch and not self.has_unfrozen:
...
trainer = Trainer(
    ...
    tokenizer=tokenizer,
    callbacks=[UnfreezeCallback(unfreeze_after_epoch=1)],  # Unfreeze after 1 epoch
)
